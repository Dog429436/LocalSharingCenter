...
namespace LocalSharingCenterHome
{
    ...
    public class ConnectedClient
    {
        ...
        public async void ListFiles()
        {
            ...
            await InterfaceHelper.WriteMessage(string.Format("{0} requested to list files", this.userId), this.serverTextBox, true);
        }
            ...
            while (true)
            {
                try
                {
                    ...
                    else if (fields[0] == Protocol.Commands.Get.ToString())
                    {
                        ...
                        await FileTransfer.SendFiles(fileNames, this.userId.ToString(), this.userId.ToString(), this.clientAesKey, this.clientAesVector, this.reader, this.writer, this.serverTextBox, null);
                    }
        ...
        public void CloseConnection()
        {
            ...
            this.stream.Close();
            this.client.Close();
            Server.clients.TryRemove(this.userId.ToString(), out _);
            InterfaceHelper.RemoveFromList(this.userId.ToString(), this.serverListBox);
        }
    }
}
